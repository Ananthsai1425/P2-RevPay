...
public class UserDAO {
    ...
    private User mapRow(ResultSet rs) throws SQLException {
        ...
        u.setWalletBalance(balance != null ? balance : BigDecimal.ZERO);
        try {
            u.setTransactionPinHash(rs.getString("txn_pin_hash"));
        } catch (SQLException e) {
            log.warn("Column 'txn_pin_hash' not found in USERS table. Transaction PIN features will be unavailable until database is updated.");
        }
    ...
    public Long save(User user) {
        String sql = "INSERT INTO USERS (full_name, email, phone, username, password_hash, txn_pin_hash, role, account_type, wallet_balance, is_active) "
                +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1)";
        try (Connection conn = dataSource.getConnection();
            ...
            ps.setString(6, user.getTransactionPinHash());
            ps.setString(7, user.getRole() != null ? user.getRole() : "USER");
            ps.setString(8, user.getAccountType());
            ps.setBigDecimal(9, user.getWalletBalance() != null ? user.getWalletBalance() : BigDecimal.ZERO);
            ps.executeUpdate();
    ...
    public void updateTransactionPin(Long userId, String pinHash) {
        String sql = "UPDATE USERS SET txn_pin_hash=?, updated_at=CURRENT_TIMESTAMP WHERE user_id=?";
        try (Connection conn = dataSource.getConnection();
                PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            log.error("Error updateTransactionPin: {}", e.getMessage(), e);
        }
    }
