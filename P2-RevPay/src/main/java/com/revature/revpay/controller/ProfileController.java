...
public class ProfileController {
    ...
    public String setTransactionPin(Authentication auth,
            @RequestParam String pin,
            @RequestParam String confirmPin,
            RedirectAttributes ra) {
        if (!pin.equals(confirmPin)) {
            ra.addFlashAttribute("errorMsg", "PINs do not match.");
            return "redirect:/profile";
        }
        if (pin.length() < 4 || pin.length() > 6) {
            ra.addFlashAttribute("errorMsg", "PIN must be 4-6 digits.");
            return "redirect:/profile";
        }
        User user = userService.findByEmailOrPhone(auth.getName());
        userService.setTransactionPin(user.getUserId(), pin);
        ra.addFlashAttribute("successMsg", "Transaction PIN updated successfully.");
        return "redirect:/profile";
    }
