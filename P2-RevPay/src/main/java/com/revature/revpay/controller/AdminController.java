...
public class AdminController {
    ...
    public AdminController(BusinessProfileDAO businessProfileDAO, LoanApplicationDAO loanApplicationDAO, 
                           ...
                           com.revature.revpay.dao.UserDAO userDAO, TransactionDAO transactionDAO) {
    ...
    public String approveLoan(@PathVariable Long id, @RequestParam String action, Authentication auth) {
        ...
        if (loan != null) {
            if ("APPROVED".equals(status)) {
                ...
                if (applicant != null) {
                    java.math.BigDecimal interestRate = loan.getInterestRate() != null ? loan.getInterestRate() : java.math.BigDecimal.valueOf(12);
                    java.math.BigDecimal interestAmount = loan.getLoanAmount()
                            .multiply(interestRate)
                            .divide(java.math.BigDecimal.valueOf(100), 2, java.math.RoundingMode.HALF_UP);
                    java.math.BigDecimal netAmount = loan.getLoanAmount().subtract(interestAmount);

                    java.math.BigDecimal newBalance = applicant.getWalletBalance().add(netAmount);
                    userDAO.updateBalance(applicant.getUserId(), newBalance);
                    
                    // Add Transaction Record for Loan Disbursement (Net Amount)
                    Transaction txn = new Transaction();
                    txn.setReceiverId(applicant.getUserId());
                    txn.setAmount(netAmount);
                    txn.setTxnType("LOAN_DISBURSE");
                    txn.setStatus("COMPLETED");
                    txn.setNote("Loan Disbursement (Net of ₹" + interestAmount + " interest): " + loan.getPurpose());
